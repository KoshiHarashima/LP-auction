...
- シナジー制約: p[2,j] <= p[0,j], p[2,j] <= p[1,j], p[2,j] >= p[0,j] + p[1,j] - 1
...
def solve_mechanism_symmetry_2agents_with_synergy(points1, weights1, solver=None):
    ...
    制約:
        ...
        - シナジーの配分制約: p[2,j] <= p[0,j], p[2,j] <= p[1,j], 
                            p[2,j] >= p[0,j] + p[1,j] - 1
    ...
    戻り値:
        ...
        - p1: np.ndarray, shape (3, J1), p1[l, j] = 参加者1が型jのときの財lの配分確率 (l=0,1,2)
        - p2: np.ndarray, shape (3, J1), p2 = p1（後方互換性のため）
    """
    J1 = len(points1)
    ...
    for i in range(J1):
        for j in range(J1):
            ...
            prob += u[i] >= u[j] + (
                ...
                + p[(2, j)] * points1_diff[i, j, 2]  # シナジーαの項
            ), f"ic_{i}_{j}"
    
    # 4. シナジーの配分制約
    # p[2,j] <= p[0,j], p[2,j] <= p[1,j]
    # p[2,j] >= p[0,j] + p[1,j] - 1
    for j in range(J1):
        # 上界制約
        prob += p[(2, j)] <= p[(0, j)], f"synergy_item0_{j}_upper"
        prob += p[(2, j)] <= p[(1, j)], f"synergy_item1_{j}_upper"
        # 下界制約（inclusion-exclusion）
        prob += p[(2, j)] >= p[(0, j)] + p[(1, j)] - 1, f"synergy_{j}_lower"
    ...
    # pの形状: (3, J1)
    p1_sol = np.zeros((3, J1), dtype=np.float64)
    for l in range(3):
...
def solve_mechanism_symmetry_2agents_with_synergy_iterative(points1, weights1, grid_sizes1,
    ...
    for iteration in range(max_iter):
        ...
        p_sol = np.zeros((3, J1), dtype=np.float64)
        for l in range(3):
...
def save_results_symmetry_2agents_with_synergy(
    ...
    パラメータ:
        ...
        u1_sol: 参加者1の効用 (np.ndarray, shape: (J1,))
        u2_sol: 参加者2の効用 (np.ndarray, shape: (J1,), 対称性によりu2 = u1)
        p1_sol: 参加者1の配分確率 (np.ndarray, shape: (3, J1))
        p2_sol: 参加者2の配分確率 (np.ndarray, shape: (3, J1), 対称性によりp2 = p1)
        obj_val: 目的関数値 (float)
        status: LPステータス (str)
